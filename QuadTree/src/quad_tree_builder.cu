...
ParallelQuadtreeBuilder::build_tree()
{
    ...
    thrust::device_vector<uint32_t> permutation_idx(x.size());
    thrust::transform(
        thrust::make_counting_iterator<uint32_t>(0),
        thrust::make_counting_iterator<uint32_t>(x.size()),
        permutation_idx.begin(),
        [] __device__ __host__ (uint32_t i) { return i; } 
    );

    auto zip_begin = thrust::make_zip_iterator(thrust::make_tuple(x.begin(), y.begin(), permutation_idx.begin()));
